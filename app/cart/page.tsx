  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
          ...
          <li key={lineKey(item)} className="flex items-center gap-4 p-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 text-xs text-muted">
                {item.variant ? `${item.variant} · ` : ""}{formatPrice(item.price)} each
              </p>
            </div>
      ...
      <div className="mt-6 flex flex-col items-end gap-3">
        ...
        <Link
          href="/checkout"
          className="rounded-lg bg-accent px-8 py-3 text-sm font-semibold text-ink transition hover:bg-accent-2"
        >
          Proceed to checkout
        </Link>
      </div>

...
type Props = {
  ...
  otherLabel?: string;
  /** Methods the owner ticked in /admin/settings — the same list the footer
   *  badges are drawn from, so the promise and the till agree. */
...
export default function CheckoutForm({
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      // variant travels as a label; the server re-prices it from the catalog
      items: items.map((i) => ({ id: i.id, qty: i.qty, variant: i.variant ?? null })),
  ...
  if (items.length === 0) {
    return (
      <div className="mx-auto max-w-xl px-4 py-20 text-center">
        ...
        <Link
          href="/shop"
          className="mt-6 inline-block rounded-lg bg-accent px-6 py-3 text-sm font-semibold text-ink transition hover:bg-accent-2"
        >
          Browse the shop
        </Link>
      </div>
      ...
      <form onSubmit={submit} className="mt-8 grid gap-8 lg:grid-cols-[1.2fr_1fr]">
        ...
        <div className="space-y-6">
          ...
          <section className="rounded-xl border border-line bg-surface p-5">
            ...
            {/* Free-text region and postcode — the store ships worldwide, so
                a fixed state list and a 4-digit pattern would reject most of
                the world's addresses. */}
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <input name="address" required placeholder="Street address" className={`${input} sm:col-span-2`} />
              <input name="suburb" required placeholder="City / town" className={input} />
              <input name="state" required placeholder="State / province / region" className={input} />
              <input name="postcode" required placeholder="Postal / ZIP code" className={input} />
              <input name="country" required placeholder="Country" autoComplete="country-name" className={input} />
              <textarea
                name="notes"
                rows={2}
        ...
        <aside className="h-fit rounded-xl border border-line bg-surface p-5">
          ...
          <div className="mt-4 space-y-1.5 border-t border-line pt-3">
            ...
            {discount > 0 && (
              <div className="flex justify-between text-sm font-semibold text-verify">
          ...
          <p className="mt-1 text-xs text-muted">
            Shipping confirmed with your order — free express over $100.
          </p>
          <button
            disabled={state === "busy"}
            className="glow-accent mt-5 w-full rounded-lg bg-accent px-6 py-3.5 text-sm font-extrabold uppercase tracking-wide text-ink transition hover:bg-accent-2 disabled:opacity-60"
          >
